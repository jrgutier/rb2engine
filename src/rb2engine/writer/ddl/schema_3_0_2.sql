-- Engine Library schema 3.0.2 (Database2/m.db)
--
-- PROVENANCE: captured from Engine DJ's OWN output, never transcribed from
-- libdjinterop (LGPL-3.0), which this project consumes only as documentation.
-- Source: /Volumes/USB DISK/Engine Library/Database2/m.db
-- Engine DJ 4.3.0.159ab27b8d migrated this database 3.0.1 -> 3.0.2 in place.
-- Captured: 2026-07-25
--
-- IMPORTANT: the same Engine DJ build (4.3.0) runs schema 3.0.1 on the desktop
-- library and 3.0.2 on this stick, so the app version does NOT determine the
-- schema version. Always resolve the target by reading Information from the
-- actual database you are about to replace.
--
-- Objects (excluding sqlite_* internals): {'table': 9, 'index': 16, 'trigger': 16, 'view': 4}
-- Track has 43 columns; 3.0.1 has 42. 3.0.2 adds albumArtSourceHash.
...
CREATE TABLE Track ( 
	...
	lastEditTime DATETIME, albumArtSourceHash CHAR(40), 
