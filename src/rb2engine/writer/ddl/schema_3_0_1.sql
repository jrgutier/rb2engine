-- Captured from Engine DJ 4.3.0.159ab27b8d (schema version 3.0.1).
-- Capture date: 2026-07-24.
-- Source: Engine-authored database tests/fixtures/golden/engine_desktop_3_0_1.db
-- (user desktop library). Captured from Engine's own output via
-- sqlite_master — NOT transcribed from libdjinterop.
...
-- Note: sqlite_sequence is omitted (SQLite internal; auto-created on first
-- AUTOINCREMENT insert). Explicit CREATE INDEX statements are included;
-- UNIQUE-constraint autoindexes are recreated by SQLite from table DDL.
